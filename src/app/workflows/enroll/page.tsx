...
export default async function EnrollCustomerPage() {
  const availableCards = (await getRepository().listCards()).filter((card) => card.status === 'AVAILABLE');
    ...
    <main className="min-h-screen bg-slate-100 p-6 text-slate-900 sm:p-8">
      <div className="mx-auto flex max-w-4xl flex-col gap-6">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            <p className="text-sm font-semibold uppercase tracking-[0.3em] text-blue-600">Registration</p>
        ...
        <form action={enrollCustomer} className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
          <div className="grid gap-4 md:grid-cols-2">
            ...
            <label className="text-sm font-medium text-slate-700">
              Phone number
              <input name="phone" required placeholder="+263771234567" className="mt-1 w-full rounded-xl border border-slate-300 px-3 py-2" />
            </label>
            <label className="text-sm font-medium text-slate-700">
              Email (optional)
              <input name="email" type="email" className="mt-1 w-full rounded-xl border border-slate-300 px-3 py-2" />
            </label>
          </div>
          ...
          <label className="mt-4 block text-sm font-medium text-slate-700">
            ...
            <span className="mt-1 block text-xs text-slate-500">
              {availableCards.length} card{availableCards.length === 1 ? '' : 's'} available in inventory
            </span>
          </label>
